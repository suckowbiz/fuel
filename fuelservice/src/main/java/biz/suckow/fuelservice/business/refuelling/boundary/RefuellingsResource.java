...
public class RefuellingsResource {
    ...
    public Response add(@OwnedVehicle @PathParam("vehicleName") final String vehicleName, final RefuellingMeta meta) {
        this.service.add(vehicleName, this.principal.getName(), meta);
    ...
    public Response list(@OwnedVehicle @PathParam("vehicleName") final String vehicleName) {
        ...
        Optional<Vehicle> vehicle = this.vehicleStore.getVehicleByNameAndOwnerEmail(this.principal.getName(), vehicleName);
        vehicle.get().getRefuellings().forEach(refuelling -> {
            String consumption = "";
