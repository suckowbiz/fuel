...
@NamedQueries({
        ...
        @NamedQuery(
                name = FuelStock.FIND_ADDITIONS_BY_VEHICLE_AND_DATE_BETWEEN,
                query = "SELECT a FROM FuelStock fs JOIN fs.additions a JOIN fs.vehicle v WHERE v = :vehicle AND a.dateAdded > :left AND a.dateAdded < :right"),
        @NamedQuery(name = FuelStock.FIND_RELEASES_BY_VEHCILE_AND_DATE_BETWEEN,
